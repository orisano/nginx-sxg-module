...
static ngx_command_t ngx_http_sxg_commands[] = {
    ...
    {ngx_string("sxg"), NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
...
static char* ngx_http_sxg_merge_srv_conf(ngx_conf_t* cf, void* parent,
  ...
  ngx_conf_merge_size_value(conf->sxg_max_payload, prev->sxg_max_payload, 64 * 1024 * 1024); // Limited to 64 MB
...
static ngx_int_t ngx_http_sxg_header_filter(ngx_http_request_t* req) {
  ...
  ngx_http_sxg_loc_conf_t* slc = ngx_http_get_module_loc_conf(req, ngx_http_sxg_filter_module);
  if (!slc->enable) {
    return ngx_http_next_header_filter(req);
  }
